	...
	<div class="app">
		...
		<div class="content">
			<div class="box main">
				...
				<br>
				<div class="tag" v-for="citem,cindex in gameDetail.categories" @click="toList(citem)" v-if="cindex == 1">{{citem}}</div>
				<h3>{{activeName}}</h3>
				<div class="starts">
					<Icon type="md-star" />
					<Icon type="md-star" />
			...
			<div class="box like">
				...
				<div class="games-con">
					<div class="games" v-for="detail,index in gameRecommond" :key="detail.name" @click="toDetial(detail.name)">
			...
			<div class="box cate">
				<h4>Categories</h4>
				<div v-for="(citem,cindex) in gameDetail.categories" @click="toList(citem)" v-if="cindex != 0">{{citem}}</div>
			</div>
		...
		methods: {
			...
			getDetail(name) {
				return new Promise((resolve,reject) => {
					this.$http.getDetail({name: name}).then(
						res => {
							resolve(res.data)
			...
			getRecommond(name) {
				return new Promise((resolve,reject) => {
				this.$http.getRecommond({name: name}).then(
					res => {
						resolve(res.data)
